...
import { DAGSnapshot, EventMessage } from '../types/orchestrator';
    ...
    if (!activeSessionId) {
      orchestratorApi.getSessions({ limit: 1 }).then((sessions) => {
        if (sessions.sessions && sessions.sessions.length > 0) {
          setActiveSessionId(sessions.sessions[0].session_id);
        }
      }).catch(console.error);
    }
  }, [activeSessionId]);
  ...
  const handleRollback = async () => {
    ...
    if (snapId) {
      try {
        ...
        alert(`Rollback completed: ${res.restored_files || 0} files restored.`);
        refreshDAG();
      } catch (err: any) {
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-slate-100 text-xs font-mono transition-colors"
