...
set "GATEWAY_BASE_URL=http://127.0.0.1:8080/v1"
...
start "Unified LLM Gateway [Port 8080]" cmd /k "title Unified LLM Gateway [Port 8080] && cd /d "%ROOT_DIR%" && set "PYTHONUTF8=1" && python -m uvicorn unified_gateway.gateway.server:app --host 127.0.0.1 --port 8080"

:: 6. Launch Agent Orchestrator Backend Server (Port 8000)
echo [2/3] Starting Agent Orchestrator Backend on http://127.0.0.1:8000 ...
start "Agent Orchestrator Backend [Port 8000]" cmd /k "title Agent Orchestrator Backend [Port 8000] && cd /d "%ROOT_DIR%" && set "PYTHONUTF8=1" && set "GATEWAY_BASE_URL=http://127.0.0.1:8080/v1" && python -m uvicorn agent_orchestrator.server:app --host 127.0.0.1 --port 8000 --reload"

:: 7. Launch Frontend IDE Dev Server (Port 3000)
echo [3/3] Starting Mission Control Frontend IDE on http://localhost:3000 ...
start "Mission Control Frontend [Port 3000]" cmd /k "title Mission Control Frontend [Port 3000] && cd /d "%ROOT_DIR%frontend" && npm run dev"
...
echo.
echo [*] Mission Control is active in background terminals.
